...
impl ActionDispatcher {
        ...
        match action {
            ...
            AppAction::SwitchBottomMode => {
                ctx.state.bottom_left_mode = match ctx.state.bottom_left_mode {
                    ...
                    crate::state::BottomLeftMode::CommitFiles => crate::state::BottomLeftMode::Parents,
            ...
            AppAction::MergePickerConfirm => {
                self.handle_merge_picker_confirm(&mut ctx)
            }
            ...
            AppAction::DiffScrollUp => self.navigation.handle(&mut ctx, NavigationAction::ScrollDiffUp),
            AppAction::DiffScrollDown => self.navigation.handle(&mut ctx, NavigationAction::ScrollDiffDown),
            ...
            AppAction::SwitchStagingFocus => self.staging.handle(&mut ctx, StagingAction::SwitchFocus),
            AppAction::StartCommitMessage => self.staging.handle(&mut ctx, StagingAction::StartCommitMessage),
            AppAction::ConfirmCommit => self.staging.handle(&mut ctx, StagingAction::ConfirmCommit),
            AppAction::CancelCommitMessage => self.staging.handle(&mut ctx, StagingAction::CancelCommit),
            ...
            AppAction::StashSelectedFile => self.staging.handle(&mut ctx, StagingAction::StashSelectedFile),
            AppAction::StashUnstagedFiles => self.staging.handle(&mut ctx, StagingAction::StashUnstagedFiles),
            ...
            AppAction::ToggleRemoteBranches => self.branch.handle(&mut ctx, BranchAction::ToggleRemote),
            ...
            AppAction::PrevSearchResult => self.search.handle(&mut ctx, SearchAction::PreviousResult),
            ...
            // Conflict legacy
            AppAction::ConflictPrevFile => self.conflict.handle(&mut ctx, ConflictAction::PreviousFile),
            AppAction::ConflictNextFile => self.conflict.handle(&mut ctx, ConflictAction::NextFile),
            AppAction::ConflictPrevSection => self.conflict.handle(&mut ctx, ConflictAction::PreviousSection),
            AppAction::ConflictNextSection => self.conflict.handle(&mut ctx, ConflictAction::NextSection),
            AppAction::ConflictSwitchPanelForward => self.conflict.handle(&mut ctx, ConflictAction::SwitchPanel),
            AppAction::ConflictSwitchPanelReverse => self.conflict.handle(&mut ctx, ConflictAction::SwitchPanel),
            AppAction::ConflictFileChooseOurs => self.conflict.handle(&mut ctx, ConflictAction::AcceptOursFile),
            AppAction::ConflictFileChooseTheirs => self.conflict.handle(&mut ctx, ConflictAction::AcceptTheirsFile),
            AppAction::ConflictChooseBoth => self.conflict.handle(&mut ctx, ConflictAction::AcceptBoth),
            AppAction::ConflictFinalize => self.conflict.handle(&mut ctx, ConflictAction::FinalizeMerge),
            AppAction::ConflictValidateMerge => self.conflict.handle(&mut ctx, ConflictAction::FinalizeMerge),
            AppAction::ConflictAbort => self.conflict.handle(&mut ctx, ConflictAction::AbortMerge),
            AppAction::ConflictLeaveView => self.conflict.handle(&mut ctx, ConflictAction::LeaveView),
            AppAction::ConflictEnterResolve => self.conflict.handle(&mut ctx, ConflictAction::EnterResolve),
            AppAction::ConflictSetModeFile => self.conflict.handle(&mut ctx, ConflictAction::SetModeFile),
            AppAction::ConflictSetModeBlock => self.conflict.handle(&mut ctx, ConflictAction::SetModeBlock),
            AppAction::ConflictSetModeLine => self.conflict.handle(&mut ctx, ConflictAction::SetModeLine),
            AppAction::ConflictToggleLine => self.conflict.handle(&mut ctx, ConflictAction::ToggleLine),
            ...
            AppAction::ConflictResultScrollUp => self.conflict.handle(&mut ctx, ConflictAction::ResultScrollUp),
            ...
            AppAction::ConflictEditCursorUp => self.conflict.handle(&mut ctx, ConflictAction::EditCursorUp),
            AppAction::ConflictEditCursorDown => self.conflict.handle(&mut ctx, ConflictAction::EditCursorDown),
            AppAction::ConflictEditCursorLeft => self.conflict.handle(&mut ctx, ConflictAction::EditCursorLeft),
    ...
    fn handle_copy_to_clipboard(&self, ctx: &mut HandlerContext) -> Result<()> {
        use crate::state::{FocusPanel, StagingFocus, BranchesSection};
        ...
        match ctx.state.view_mode {
            ViewMode::Graph => {
                ...
                match ctx.state.focus {
                    FocusPanel::Files => {
                        if let Some(file) = ctx.state.commit_files.get(ctx.state.file_selected_index) {
                            text_to_copy = file.path.clone();
                            if let Some(ref diff) = ctx.state.selected_file_diff {
                                let diff_text = diff.lines.iter()
            ...
            ViewMode::Staging => {
                match ctx.state.staging_state.focus {
                    StagingFocus::Unstaged => {
                        text_to_copy = ctx.state.staging_state.unstaged_files()
                            .get(ctx.state.staging_state.unstaged_selected())
                            .map(|f| f.path.clone())
                            .unwrap_or_default();
                    }
                    StagingFocus::Staged => {
                        text_to_copy = ctx.state.staging_state.staged_files()
                            .get(ctx.state.staging_state.staged_selected())
                            .map(|f| f.path.clone())
                            .unwrap_or_default();
                    }
                    StagingFocus::Diff => {
                        text_to_copy = ctx.state.staging_state.current_diff.as_ref()
                            .map(|diff| {
                                diff.lines.iter()
                                    .map(|line| line.content.trim_end_matches('\n').to_string())
                                    .collect::<Vec<_>>().join("\n")
            ...
            ViewMode::Branches => {
                match ctx.state.branches_view_state.section {
                    BranchesSection::Branches => {
                        text_to_copy = ctx.state.branches_view_state.selected_branch()
                            .map(|b| b.name.clone())
                            .unwrap_or_default();
                    }
                    BranchesSection::Worktrees => {
                        text_to_copy = ctx.state.branches_view_state.worktrees.selected_item()
                            .map(|w| format!("{}: {}", w.name, w.path))
                            .unwrap_or_default();
                    }
                    BranchesSection::Stashes => {
                        text_to_copy = ctx.state.branches_view_state.stashes.selected_item()
                            .map(|s| format!("{}: {}", s.oid.to_string().get(0..7).unwrap_or(""), s.message))
                            .unwrap_or_default();
                    }
                }
            }
        ...
        if !text_to_copy.is_empty() {
            ...
            clipboard.set_text(&text_to_copy)
                .map_err(|e| crate::error::GitSvError::Clipboard(e.to_string()))?;
            ctx.state.set_flash_message("Copié dans le presse-papier ✓".to_string());
        }
    ...
    fn handle_merge_picker_confirm(&self, ctx: &mut HandlerContext) -> Result<()> {
        ...
        let branch_to_merge = ctx.state.merge_picker.as_ref()
        ...
        if let Some(branch_name) = branch_to_merge {
            match crate::git::merge::merge_branch_with_result(&ctx.state.repo.repo, &branch_name) {
                Ok(MergeResult::UpToDate) => {
                    ctx.state.set_flash_message(format!("Branche '{}' est déjà à jour", branch_name));
                }
                Ok(MergeResult::FastForward) => {
                    ctx.state.set_flash_message(format!("Fast-forward vers '{}'", branch_name));
                    ctx.state.mark_dirty();
                }
                Ok(MergeResult::Success) => {
                    ctx.state.set_flash_message(format!("Branche '{}' mergée avec succès", branch_name));
                    ctx.state.mark_dirty();
                }
                Ok(MergeResult::Conflicts(conflicts)) => {
                    ...
                    let current = ctx.state.current_branch.clone().unwrap_or_else(|| "HEAD".to_string());
            ...
            match confirm_action {
                ConfirmAction::DiscardAll => {
                    ...
                    if let Err(e) = crate::git::discard::discard_all(&ctx.state.repo.repo) {
                        ...
                        ctx.state.set_flash_message("Modifications ignorées ✓".to_string());
                    }
                    ctx.state.mark_dirty();
                }
                ...
                ConfirmAction::BranchDelete(name) => {
                    ...
                    if let Err(e) = crate::git::branch::delete_branch(&ctx.state.repo.repo, &name) {
                        ...
                        ctx.state.set_flash_message(format!("Branche {} supprimée ✓", name));
                    }
                    ctx.state.mark_dirty();
                }
