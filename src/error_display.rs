...
pub fn format_error_message(err: &GitSvError) -> String {
    match err {
        ...
        GitSvError::OperationFailed { operation, details } => {
            format!("❌ {operation} échoué: {details}")
        }
        GitSvError::BranchNotFound { name } => format!("❌ Branche '{name}' non trouvée"),
        GitSvError::FileNotFound { path } => format!("❌ Fichier '{path}' non trouvé"),
